...
export default function Home() {
  return (
    <SiteShell>
      ...
      <Box bg="brand.navy" color="brand.white" py={{ base: 16, md: 22 }}>
        <Container maxW="1240px">
          <Grid templateColumns={{ base: '1fr', lg: '.75fr 1.25fr' }} gap={{ base: 10, lg: 16 }} alignItems="start">
            ...
            <SimpleGrid columns={{ base: 1, sm: 2 }} spacing="1px" bg="rgba(255,255,255,.12)">{pillars.map(pillar => <Box key={pillar.number} bg="brand.navy" p={7} minH="170px"><Heading mt={7} fontSize="27px" fontWeight="400">{pillar.title}</Heading><Text mt={3} color="brand.gold" fontSize="16px" letterSpacing=".06em" textTransform="uppercase">{pillar.group}</Text></Box>)}</SimpleGrid>
          </Grid>
        </Container>
      </Box>
      ...
      <Box py={{ base: 16, md: 22 }}><Container maxW="1240px"><Eyebrow>Initiatives</Eyebrow><Heading mt={4} mb={10} fontSize={{ base: '40px', md: '56px' }} fontWeight="400">Ideas, brought to life.</Heading><Stack spacing={0}>{initiatives.map((item) => <Grid key={item.number} templateColumns={{ base: '1fr', md: '.8fr 1.2fr' }} gap={{ base: 3, md: 8 }} py={8} borderTop="1px solid" borderColor="brand.mist" alignItems="start"><Heading fontSize="20px" fontWeight="400">{item.title}</Heading><Text fontSize="20px" color="#625b52" lineHeight="1.8">{item.description}</Text></Grid>)}</Stack><Button as={NextLink} href="/initiatives" mt={8} variant="outlineGold" color="brand.saffron" borderColor="brand.saffron" rightIcon={<FiArrowRight />}>Explore all initiatives</Button></Container></Box>
