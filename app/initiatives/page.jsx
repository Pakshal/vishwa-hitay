...
export default function InitiativesPage() { return <SiteShell><PageHeader eyebrow="Initiatives" title="Ideas, brought to life.">Programmes designed to move civilisational wisdom from reflection into meaningful practice.</PageHeader><Container maxW="1180px" py={16}>{initiatives.map(item => <Grid key={item.number} templateColumns={{ base: '1fr', md: '.8fr 1.2fr' }} gap={7} py={9} borderTop="1px solid" borderColor="brand.mist"><Heading fontSize="20px" fontWeight="400">{item.title}</Heading><Text fontSize="20px" color="#625b52" lineHeight="1.8">{item.description}</Text></Grid>)}</Container></SiteShell>; }
